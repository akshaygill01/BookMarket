...
//    login
   ...
   @PostMapping("/activate-account")
    public ResponseEntity<Response> activateAccount(@RequestParam String token) {
        log.trace("Received activate account request , token: {}", token);
       try {
           return ResponseEntity.ok().body(authenticationService.activateAccount(token));
       } catch (Exception e) {
           log.error(e.getMessage());
           return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Response.error(e.getMessage()));
       }
